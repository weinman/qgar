...

{
  ...
  checkRange(_pPixMap, _width, BoundingBox(Point(), _width, _height));
}
...
{
  // Ensure that passed pixmap does not contradict image invariant.
  checkRange(anImg.pPixMap(), _width, BoundingBox(Point(), _width, _height));
...

{
  ...
  checkRange(_pPixMap, _width, aBox);
}
				   ...
				   T aPixVal)
{
  *(_pPixMap + (aY * _width) + aX) = aPixVal;
  ...
  checkRange(_pPixMap, _width, BoundingBox(aX, aY, 1, 1));
}
				 ...
				 const T* const aRow)
{
  ...
  checkRange(_pPixMap + (aRowIdx * _width), _width, 
				    ...
				    const T* const aCol)
{
  ...
  checkRange(_pPixMap, _width, BoundingBox(Point(), 1, _height));
}
...
GenImage<T, CheckPolicy>::operator=(const GenImage<T, OtherCheckPolicy>& anImg)
{
  ...
  checkRange(anImg.pPixMap(), anImg.width(), 
